...
read_and_filter_data <- function(start_time, end_time){
  ...
  path_to_raw <- list.files('results_calibrated_averaged_data', 'ten', full.names = T)
...
get_data_for_timestamp <- function(calibrated_data, timestamp, pollutant_of_interest){
  ...
  frame_data <- filter(calibrated_data, Time == timestamp) %>%
    ...
    mutate(pollutant = pmax(pollutant, 0)) %>%
    na.omit()
...
```{r}
make_pollution_video('PM25', '2020-09-10 00:00:00', '2020-09-10 02:00:00')
```
