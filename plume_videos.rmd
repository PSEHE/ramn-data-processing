...
get_wind_data <- function(){
  df_10m <- read_csv("wind_10m_avg.csv") %>% 
  mutate(Date = mdy(Date)) %>% 
  mutate(date_time = as.POSIXct(paste(Date, Time_10m), format="%Y-%m-%d %H:%M:%S")) %>% 
  select(Station, date_time, Longitude, Latitude, windspd_10m_avg, winddir_10m_avg)
...
make_pollution_video <- function(pollutant_of_interest, start_time, end_time){
  ...
  wind_arrows <- get_wind_arrow(start_time, end_time, line.length, arrow.length, arrow.angle)
  
  sp.lines.df <- get_spatial_lines(df, start_time, end_time)
