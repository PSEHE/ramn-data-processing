...
read_and_calibrate_data <- function(start_time, end_time){
  ...
  raw_data <- read.csv('results_downloader_data/PSE_AQY_2019-12-06_2021-01-04_freq_10.csv') %>% #update to local 
    filter(Time >= start_time & Time <= end_time)
  
  cal_params <- read.csv('pse_calvals_2021-04.csv') #update to local
  ...
  calibrated_data <- inner_join(raw_data, cal_params, by = 'ID') %>%
    ...
    rename('O3_raw'='O3', 'PM25_raw'='PM2.5', 'NO2_raw'='NO2', 'Ox_raw'='Ox') %>%
...
```{r}
make_mapframe <- function(calibrated_data, timestamp, pollutant_of_interest, raster_grid, raster_coords){
...
animate_mapframe_timeseries <- function(pollutant_of_interest, start_time, end_time){
  ...
  mapframe_paths <- list.files('results', '*.png', full.names = T)
...
make_pollution_video <- function(pollutant_of_interest, start_time, end_time){
  ...
  lapply(timestamps, make_mapframe, calibrated_data=calibrated_data, pollutant_of_interest=pollutant_of_interest, raster_grid=raster_grid, raster_coords=raster_coords)
  
  animate_mapframe_series(pollutant_of_interest, start_time, end_time)
}
```

```{r}
make_pollution_video('PM25', '2020-08-20 00:00:00', '2020-08-20 01:00:00')
```
