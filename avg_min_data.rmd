...
average_minute_data <- function(averaging_time_str){
  ...
  avg_data <- group_by(min_data, ID, avg_timestamp) %>%
    ...
    summarize(PM25_raw = mean(Data.PM2.5.raw, na.rm = T),
              PM25_precal = mean(Data.PM2.5, na.rm = T), 
              O3_precal = mean(Data.O3, na.rm = T), 
              NO2_precal = mean(Data.NO2, na.rm = T), 
  ...
  data_cal <- data_calvals %>%
    mutate(Ox_precal = NO2_precal + 1.1*O3_precal,
           Ox = Ox.gain*(Ox_precal - Ox.offset), 
           O3 = O3.gain*(O3_precal - O3.offset), 
           NO2_int = Ox - NO2.a.value*O3, 
           NO2 = NO2.gain*(NO2_int - NO2.offset),
           PM25 = PM.gain*(PM25_precal - PM.offset)) %>%
    select(ID, Time, TEMP, RH, DP, PM25, PM25_precal, PM25_raw, O3, O3_precal, Ox, Ox_precal, NO2, NO2_precal)

  data_cal_round <- mutate_at(data_cal, .vars = c('PM25', 'PM25_precal', 'PM25_raw', 'O3', 'O3_precal', 'Ox', 'Ox_precal', 'NO2', 'NO2_precal'), .funs = ~round(.x, 3))
...
#write.csv(calibrated_60min, 'results_calibrated_averaged_data/AQY633_642_60min_Sac.csv', row.names = F)
```
